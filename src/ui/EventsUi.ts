...
export class EventsUi {
    private eventContain: HTMLElement;
    private eventForm: HTMLFormElement;
    private message: HTMLElement;

    constructor(
        private eventsService: EventsServices,
        private onInscClick : (eventId:string) =>void
    
    ) {
        this.eventContain = document.getElementById('events-list')!;
        this.eventForm = document.getElementById('event-form') as HTMLFormElement;
    ...
    manageSubmition(e: Event) {
        e.preventDefault();

        const titre = (document.getElementById('event-titre') as HTMLInputElement).value;
        ...
        const lieu = (document.getElementById('event-lieu') as HTMLInputElement).value;
        const categorie = (document.getElementById('event-categorie') as HTMLSelectElement).value;
        const capaciteStr = (document.getElementById('event-capacite') as HTMLInputElement).value;

        const capacite = +capaciteStr;
        const date = new Date(dateStr);

        const rslt = this.eventsService.createEvent(
            titre,
            description,
            date,
            lieu,
    ...
    displayMessage(message: string, success: Boolean): void {
    ...
    creerCarteEvenement(event: Events) : string{
